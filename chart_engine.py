...
STYLE_RECOMMENDATION_SYSTEM_PROMPT = (
    ...
    "The same fresh-each-time reasoning applies to the garment type itself, not just color: a "
    "real, reported pattern for warm-weather, active occasions specifically has been a tank top "
    "paired with shorts or loose pants, request after request, regardless of how different the "
    "actual occasions were\u2014as if that one combination were the only way to satisfy "
    "\"breathable and free to move in.\" It isn't, and it shouldn't be treated as the default "
    "answer whenever those needs come up. Reason through what this specific person, occasion, "
    "and chart actually call for each time, the same way color has to be decided fresh rather "
    "than defaulted to.\n\n"
