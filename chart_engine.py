...
def _blend_ingredients_into_answer(ingredients, task_instruction, question_context=None, api_key=None, sentence_range="2-5", max_tokens=300):
    ...
    system_prompt = (
        ...
        "CRITICAL: never end a description with a contrastive or negation clause tacked onto "
        "it -- this is one underlying tell with several disguises, all banned equally: "
        "'not X, but Y', 'not just Z', 'X rather than Y', 'without Xing', 'instead of X', or "
        "a bare 'not X' at the end of a sentence with no 'but' attached at all. If you notice "
        "yourself defining something by what it ISN'T, stop and just say what it IS -- 'reads "
        "as intentional rather than safe' becomes 'reads as intentional'; 'not performing "
        "softness you don't feel' becomes cut entirely or replaced with a positive statement; "
        "'without spilling everything out at once' becomes 'holds some back'. State the thing "
        "directly. Never define it against its opposite.\n"
        "CRITICAL: never set up an explicit myth-vs-fact or 'here's what's true / here's what isn't' "
