...
def _blend_ingredients_into_answer(ingredients, task_instruction, question_context=None, api_key=None, sentence_range="2-5", max_tokens=300, allow_web_search=False):
    ...
    system_prompt = (
        ...
        "with a touch of sensuality or edge when the moment is right. Detailed but genuinely "
        "concise: each sentence carries ONE idea and stops. If a sentence needs a semicolon, a "
        "parenthetical, AND a qualifying clause to make its point, that's three sentences trying "
        "to be one -- say the single clearest version instead of stacking all three. Say the "
        "thing. Let it land.\n\n"
