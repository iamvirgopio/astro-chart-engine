...
def _blend_ingredients_into_answer(ingredients, task_instruction, question_context=None, api_key=None, sentence_range="2-5", max_tokens=300, allow_web_search=False, interpretive=False, stylist_voice=False, model="claude-haiku-4-5-20251001"):
    ...
    system_prompt = (
        ...
        + (
            ...
            "says about you, and don't end with a line summarizing the whole look.\n\n"
            ...
            "The placements below each give you a general vibe or tendency, not a fixed set "
            "of items to choose from\u2014any specific colors, fabrics, or pieces mentioned in "
            "parentheses are illustrative examples only, marked with \"etc.\" on purpose, not "
            "a closed list. Invent the actual, specific outfit yourself: real, concrete items "
            "(an exact garment, an exact color, an exact fabric) that fit both the vibe "
            "described and the specific occasion given, genuinely creating a new combination "
            ...
            "A real, reported failure: the same person's chart placements are fixed and get "
            "sent identically on every single request they make, so if you let a vibe's fabric "
            "weight or structure win by default, every outfit for that person converges on the "
            "same silhouette regardless of what they actually asked for\u2014a heavy layered "
            "top-plus-outer-layer-plus-boots look showing up for a cookout, a market run, and a "
            "dinner alike, just with different colors swapped in. When the occasion and a "
            "vibe's typical fabric weight or structure genuinely conflict\u2014a cozy, "
            "heavyweight vibe paired with a warm-weather or physically active occasion, for "
            "instance\u2014the occasion wins on practical decisions: fabric weight, sleeve and "
            "hem length, how many layers, footwear type. The vibe still shapes which specific "
            "colors, cuts, and finishes you choose within whatever the occasion actually calls "
            "for, but it never overrides what the occasion itself requires. A cookout, a hike, "
            "a beach day, and a black-tie dinner should never produce the same CATEGORY of "
            "outfit just because the underlying chart placements are identical across all four "
            "requests\u2014vary the actual type of garment (a dress versus separates, "
            "structured versus relaxed, layered versus a single easy piece) as much as the "
            "occasion itself varies, not just the color or trim.\n\n"
