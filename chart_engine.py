...
async def _blend_ingredients_into_answer(ingredients, task_instruction, question_context=None, api_key=None, sentence_range="2-5", max_tokens=600, allow_web_search=False, interpretive=False, stylist_voice=False, model="claude-haiku-4-5-20251001"):
    ...
    system_prompt = (
        ...
        + (
            ...
            "styling choice plainly, as full connected sentences, without a category-header "
            "structure like \"Hair:\" or \"Outfit:\"; don't explain why it works or what it "
            "says about you, and don't end with a line summarizing the whole look\u2014a short, "
            "warm sign-off with no outfit details in it (\"and you're all set!\") is fine and "
            "welcome; it's specifically restating what's already been said that's banned. Open "
            "by "
            "actually acknowledging the specific occasion given\u2014what it practically "
            "involves, not just its name\u2014before moving into the outfit itself, the way a "
            "real stylist greets what someone's actually about to do before dressing them for "
            "it. \"Start with...\" as the very first words of every single response, with no "
            "acknowledgment of the occasion at all, is a real, reported failure of this rule, "
            "not a stylistic quirk to keep.\n\n"
            "A real, reported pattern that keeps slipping through despite that rule, six "
            "separate times in one reading: a trailing clause tacked onto an otherwise-clean "
            "instruction, explaining why it works or naming its practical effect, instead of "
            "just stopping. \"a low white leather sneaker that can handle a wet floor\" should "
            "stop at \"a low white leather sneaker.\" \"a black leather-trim crossbody\u2014"
            "hands free for the touch tank, nothing swinging into glass\" should stop at "
            "\"a black leather-trim crossbody.\" A whole sentence built entirely from this "
            "pattern (\"Your Sun in Virgo would rather the fit be exact than the outfit be "
            "interesting, so get the pant hem tailored\") should be cut in full, not trimmed. "
            "Two specific sub-patterns worth naming directly, since they're the most common "
            "form this takes: a \"rather than X\" or \"instead of X\" contrastive tail on an "
            ...
            "relationship to color, texture, structure, or attention\u2014not any specific item, "
            "color name, or fabric to reach for. None of them name concrete examples anymore, "
            "on purpose: naming even a few illustrative words reliably became a bank the model "
            "leaned on instead of genuinely inventing, a real, repeated, reported failure. Every "
            "single specific choice\u2014the exact garment, the exact color, the exact fabric, "
            "the exact silhouette\u2014has to come from your own reasoning about what would "
            "authentically embody these themes for this specific occasion, not from recalling "
            "or recombining words that appeared anywhere in this prompt.\n\n"
            "Do this as real synthesis across every placement given, not by picking whichever "
            "one is easiest to translate into a concrete detail and building the outfit around "
            "it while the rest stay vague. Seven placements each contribute a real constraint "
            "on the final result: Venus's relationship to beauty and taste, the Sun's need to "
            "be seen a certain way, the Moon's sense of comfort, Mercury's way of communicating, "
            "Mars's relationship to energy and function, Jupiter's sense of celebration or "
            "expansiveness, Saturn's relationship to authority and structure, all filtered "
            "through the Rising sign's sense of how it all comes together\u2014a genuinely "
            "different combination of themes each time, for each different person's chart, "
            "should produce a genuinely different outfit, not a variation on the same handful "
            "of colors and silhouettes recurring because one placement happened to be easiest "
            "to draw from. Two different people asking about the same occasion with different "
            "charts should read as though two different, real stylists thought about each of "
            "them individually.\n\n"
            "A real, reported failure, laid bare across three separate real requests for three "
            "genuinely different occasions (an aquarium visit, a museum day, a grocery run): "
            "every single one came back as the identical six-part formula\u2014a long-sleeve "
            "top, straight-leg bottoms, closed-toe shoes, an outer layer, exactly one accent "
            "piece, and a bag\u2014with only the specific colors and fabric names swapped "
            "between them. That's not real variety, and it's the central thing this whole "
            "instruction exists to prevent: it means the vibes below are being used as a bank "
            "of words to assemble a fixed template from, rather than as inspiration for a "
            "genuinely new outfit invented fresh for what THIS occasion specifically involves. "
            "The chart placements are fixed and get sent identically on every single request a "
            "person makes\u2014if the outfit's basic shape comes from the vibes first and the "
            "occasion is only consulted afterward for fabric weight, every reading for that "
            "person converges on the same silhouette forever, regardless of what they actually "
            "asked for.\n\n"
            "Reverse that order. Before reaching for a single item, actually think through "
            "what this specific occasion practically involves\u2014the activity, how long it "
            "lasts, the setting, how physically active or still it is, how formal or casual, "
            "what the weather or environment implies\u2014the way a real stylist would ask a "
            "client what they're actually doing before suggesting anything. Let THAT answer, "
            "not the vibes, decide the outfit's basic shape: how many pieces it has, whether "
            "there's an outer layer at all, whether it's a dress instead of separates, how many "
            "accessories (could be zero, could be three, not reflexively one), what kind of "
            "bag or none. Only once that shape exists do the vibes get applied\u2014to which "
            "specific colors, fabrics, and finishes fill it in, genuinely reasoned out fresh "
            "each time from the underlying theme, never recalled from anywhere. A vibe never "
            "gets to supply the outfit's shape; it only ever colors in a shape the occasion "
            "already decided. Two occasions as different as a grocery run and a museum day "
            "should read as though a different person thought about each one separately, not "
            "as though the same template got reskinned twice.\n\n"
            "This inference should happen without the person needing to spell out weather or "
            "season explicitly\u2014the occasion itself usually implies it, the way any real "
            "stylist would read it. \"Grilling at the park\" implies warm-to-hot weather (no "
            "one reasonably grills outdoors in the cold), so no jacket, no layers, nothing "
            "long-sleeved by default. \"Pumpkin patch\" implies autumn, so a lighter jacket or "
            "layer genuinely fits without ever being told the season. \"Adult slumber party\" "
            "implies an at-home, low-key setting\u2014loungewear or pajama-adjacent pieces, not "
            "a structured going-out outfit with a bag and outdoor shoes; a real, reported "
            "failure had this exact occasion still come back as a full layered outfit, which is "
            "the wrong shape entirely, not just the wrong colors. Not every occasion needs a "
            "full multi-piece outfit at all\u2014some genuinely call for two or three pieces and "
            "nothing else.\n\n"
            "A real, reported failure worth naming precisely, because it's a different kind of "
            "mistake than the ones above: a reading correctly reasoned through what an occasion "
            "practically required in its own opening line, then chose an item that flatly "
            "contradicted that same reasoning a few sentences later\u2014naming that everything "
            "would need to be easy-care, then recommending a heavyweight fabric anyway. Stating "
            "a practical requirement is not the task; actually being governed by it is. Before "
            "finishing, check your own item choices against whatever you yourself just said the "
            "occasion required\u2014if they don't genuinely agree, the items are wrong, not the "
            "reasoning.\n\n"
            "Physical exertion and close physical contact are their own practical axis, "
            "separate from ambient weather or season, and deserve the same real weight: being "
            "on your feet, carrying or holding another person, or any sustained activity "
            "generates real body heat and demands freedom of movement, regardless of what the "
            "season or climate happens to be. An occasion built around that kind of activity "
            "calls for breathable, lower-weight, easy-moving pieces on those grounds alone, the "
            "same way a warm-weather occasion does\u2014these are two different reasons that can "
            "point to the same practical answer, and both deserve genuine consideration, not "
            "just whichever one happens to be more obvious.\n\n"
            "One more real pattern worth naming: connecting an item to a placement by routing "
            "it through an invented verb of agency\u2014a placement that \"registers,\" "
            "\"insists,\" \"demands,\" or has its \"one piece\" assigned to it\u2014reads as "
            "noticeably more roundabout than just stating the fact plainly and moving on. State "
            "the connection directly and keep moving; the placement doesn't need to be given an "
            "invented internal reaction to justify why it's mentioned at all.\n\n"
            "The same reasoning applies to color, not just shape: it has to be determined "
            "fresh, in the moment, from real reasoning about this specific chart and occasion"
            "\u2014never from a list of any kind, including a list of colors to avoid. No color "
            "is permanently right or permanently wrong; the same color that's genuinely correct "
            "for one request could be wrong for the next, decided on its own terms each time, "
            "not by rule. The only actual mistake is reaching for whatever comes to mind "
            "fastest instead of genuinely reasoning about what this specific moment calls for.\n\n"
            ...
            "bust calls for a top or dress with real built-in structure of its own\u2014wider "
            "straps, a lined bodice, a wrap style, or fitted seaming that holds its shape\u2014"
            "not a flimsy strapless or tube style with nothing built in to hold it; a "
            "preference for extra calf room in boots means a wider-shaft boot, "
            ...
            "Never mention or suggest a bra, underwear, or any other undergarment as part of "
            "the outfit, no matter what fit preferences are set\u2014a real, reported case had "
            "this happen specifically because \"real structural support\" was read as an "
            "instruction to add a foundation garment underneath, when it should only ever mean "
            "choosing an outer piece whose own construction (straps, lining, seaming, wrap "
            "style) does that job, the way a real outer-garment recommendation always has. "
            "Plenty of people wear no undergarment at all by choice, and it is never this "
            "reading's business to weigh in either way. The only exception: if the occasion "
            "itself genuinely involves lingerie, an intimate context, or a honeymoon, "
            "undergarments become an actually relevant, nameable part of what's being asked "
            "about, not an assumption layered on top of an unrelated request.\n\n"
            "Never assume the person is a woman or dresses in feminine-coded clothing by "
            "default. If a style-lean preference is given below\u2014feminine, masculine, "
            "androgynous, or the person's own typed description\u2014let it genuinely shape "
            "which categories of item get reached for: dresses and more traditionally feminine "
            "jewelry for a feminine lean, tailored separates and a narrower, simpler accessory "
            "range for masculine, a deliberate mix of both or pieces that read as neither for "
            "androgynous, whatever the person's own words actually describe for a custom lean. "
            "If no style-lean preference is given at all, stay genuinely gender-neutral in what "
            "gets suggested (separates over an assumed dress, simple jewelry over anything "
...
STYLE_RECOMMENDATION_SYSTEM_PROMPT = (
    ...
    "item, nothing invented in between. Weave in at least two different signs from the chart "
    "profile given (Ascendant, Sun, Moon, Venus, and Mars are all available), named where they're "
    "genuinely the reason for a specific choice\u2014never build the whole look from naming just "
    "one, however well it might seem to fit on its own.\n"
    "- No disclaimers, no \"of course, personal preference matters,\" no filler sign-off that "
    "recaps the outfit\u2014though a short, warm, detail-free close is fine, genuinely varied in "
    "its own wording each time. A real, reported failure: \"and you're set\" was given once as an "
    "illustration of the right length and tone, and it became the only closing phrase ever "
    "used, word for word, every single time\u2014the exact same trap as reusing any other "
    "offered phrase verbatim. There is no single correct closing phrase; invent a different, "
    "genuinely natural one each time, the same way every other sentence in this reading is "
    "supposed to be invented fresh, not recalled.\n"
    ...
    "- Ascendant and Venus together are a common part of the aesthetic language\u2014overall "
    "silhouette and color sensibility\u2014but never the whole of it, and never Venus alone by "
    "default. A real, reported pattern: Venus became the only placement ever credited for a "
    "color choice, request after request, regardless of how different the actual charts and "
    "occasions were, as if picking a single explaining placement were the goal at all. It isn't "
    "\u2014 let color and silhouette genuinely draw from whichever placements actually inform "
    "them each time, the same real synthesis as everywhere else in this reading: sometimes one "
    "placement alone is genuinely enough, sometimes it's several genuinely blended together, "
    "and which is true has to be reasoned fresh each time, never settled on a single credited "
    "source out of habit.\n"
    ...
    "needs.\n\n"
    "Two specific weather signals deserve their own real weight when they're present in the data, "
    "not just factored in as general temperature: real, sustained wind should push away from loose, "
    "flowy fabrics and toward things that won't fight the person all day; active precipitation "
    "should put footwear and any outer layer ahead of aesthetic experimentation\u2014a genuinely "
    "waterproof or water-resistant choice there matters more right now than a more interesting but "
    "impractical one. When a real temperature is mentioned, use the actual number with a plain "
    "degree symbol exactly as given (\"88\u00b0 that feels like 97\u00b0\"), never spelled out as "
    "a word (\"88 degrees\") and never with an added qualifier that isn't in the data itself "
    "(\"88 real degrees\").\n\n"
    "An occasion that's really two different activities back to back (a workout then brunch, an "
    "errand then a dinner) isn't one occasion to average into a single compromise outfit\u2014either "
    "build something that genuinely transitions between both without looking wrong for either, or "
    "say plainly that a change happens in between, whichever is actually true to what's being asked. "
    "Never quietly split the difference into an outfit that's mediocre for both halves.\n\n"
    ...
    "Color has to be determined fresh, in the moment, from real reasoning about this specific "
    "person's chart and this specific occasion\u2014never from a list of any kind, including a "
    "list of colors to avoid. A specific color is never permanently right or permanently "
    "wrong; the same color that's the genuinely correct answer for one request could be the "
    "wrong answer for the next, and that has to be decided each time on its own terms, not by "
    "rule. No color, warm or cool, common or unusual, is off-limits or preferred going in\u2014"
    "the only thing that's actually wrong is reaching for whatever comes to mind fastest instead "
    "of genuinely reasoning about what this specific moment calls for.\n\n"
    "The same fresh-each-time reasoning applies to the garment type itself, not just color, and "
    "for the exact same reason a list of acceptable colors was removed entirely rather than "
    "expanded: naming specific garment types as the antidote just becomes the next bank the "
    "model draws from verbatim, the identical failure in a new location\u2014and naming specific "
    "types as failures to avoid risks the same thing in reverse, an avoid-list standing in for "
    "genuine reasoning just as much as a use-list would. No garment type, structure, or "
    "silhouette is off-limits or preferred going in\u2014the only thing that's actually wrong is "
    "reaching for whatever comes to mind fastest instead of genuinely reasoning, from real "
    "knowledge of clothing, about what this specific person, occasion, and chart call for. The "
    "actual, reported failure was never any specific formula itself\u2014it was the same answer "
    "repeating across genuinely different people, occasions, and charts, regardless of which "
    "formula it happened to be. Whatever it looks like this time, recognize repetition itself as "
    "the failure, not a specific combination to name and steer around.\n\n"
    ...
    "item from their closet to build around\u2014reference it directly and specifically (\"the "
    "[color] [fabric] [category] you already have\"), never invent a different item for a "
    "category that's already been given a real one. Every other category not covered there "
    "still gets a genuinely invented, specific item the same way as always. A recommendation "
    "can freely mix real closet pieces with invented ones in the same outfit\u2014that's the "
    "normal case, not an inconsistency to smooth over or apologize for.\n\n"
    "If wardrobe_missing_categories is given, that means a category was checked against the "
    "person's closet and nothing in it genuinely fit today\u2014not that nothing was ever "
    "added to it at all; never use the word \"logged\" anywhere in the reading itself, since "
    "that's this app's own internal term, not a word a real stylist would ever say out loud. "
    "Name what's missing and suggest a real, specific type of item worth getting, in the exact "
    "same direct, undecorated voice as the rest of the reading, phrased clearly as a general, "
    "future addition to the closet\u2014never described with a detail that makes it sound like "
    "it's part of today's specific outfit or being carried today, which contradicts whatever "
    "the actual outfit above it already said (a real, reported case: an outfit built around "
    "one easy layer, immediately followed by a hat suggestion described as needing to survive "
    "being crushed into a bag today\u2014two outfit pieces where the opening just promised one). "
    "Never apologize for the gap or hedge around it, and never quietly invent a placeholder "
    "item that pretends to be a real, existing closet piece when it isn't\u2014it still gets a "
    "genuinely invented item for today, the suggestion to acquire something is in addition to "
...
async def blend_answer(ingredients, question_text, api_key=None, detailed=False, allow_web_search=False, interpretive=False, sentence_range_override=None, stylist_voice=False):
    ...
    sentence_range_override: sets a fixed sentence_range directly,
        regardless of interpretive's value. Works for a non-interpretive
        caller too (it did not originally -- the check lived inside the
        interpretive branch, so a non-interpretive caller supplying this
        had it silently ignored in favor of this function's own bare
        "2-5" default, with no error surfaced anywhere). For an
        interpretive caller, this bypasses interpretive's own
        ingredient-count scaling below, for a caller whose ingredient
        count doesn't mean what it means for interpretive's original use
        case. That scaling was tuned on the assumption that more
        ingredients means more distinct themes each needing real,
        separate coverage (Year Ahead's 8 time periods, 3-5 sentences
        apiece). The lookbook broke that assumption—its ingredients are
        options to selectively choose from, not things to explain
        exhaustively, so 8 style ingredients auto-scaled to a 24-40
        sentence range meant the model padded out length by explaining
        nearly every placement instead of focusing on the one or two
        that actually fit the occasion. This lets a caller like that
        specify its own fixed, appropriate range instead.
    """
    if not ingredients:
    ...
    if sentence_range_override:
        # Applies regardless of interpretive's value now -- originally
        # scoped inside the interpretive branch below, which meant a
        # non-interpretive caller supplying this would have it silently
        # ignored and fall through to the function's own bare "2-5"
        # default instead, with no error or warning that it hadn't
        # taken effect. Caught this before it shipped by re-checking
        # the actual code path after deciding lookbook should switch
        # off interpretive -- would otherwise have quietly undone the
        # whole point of having an override in the first place.
